...
// 2. A typed local of a type with no scalar methods this pass (float) is excluded:
//    normal "method on float" error. (string and int DO desugar now; float/bool are a
//    same-mechanism follow-on and are intentionally not handled yet.)
try {
    float $i = 5.0;
    $i->foo();
} catch (\Error $e) {
