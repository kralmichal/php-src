...
namespace {
    ...
    // (2) A `bool` typed property is excluded: bool has no scalar-method backing class (its
    // operations are language operators, not methods), so the normal path errors on the bool.
    // (A `string` property dispatches to Str, an `int` to Int, a `float` to Float — see those tests.)
    class BoolProp {
        public bool $n = true;
        public function f() { return $this->n->trim(); }
    }
...
Call to a member function trim() on true
