...
/* Tier 2 receiver: a call whose result is *provably* a non-nullable scalar of exactly
 * `scalar_type` (IS_STRING or IS_LONG), resolvable at compile time. Handles three call
 * forms:
 *   - `$this->m()`        — method on the current class (resolved via its method
 *                            table). An overriding method in a subclass may actually
 *                            run, but return-type variance forbids widening the declared
 *                            return, so the result is guaranteed the same non-nullable
 *                            scalar.
 ...
 * non-nullable-`scalar_type` return falls through. */
static bool zend_call_ast_returns_nonnullable_scalar(zend_ast *ast, uint8_t scalar_type) /* {{{ */
{
	if (ast->kind == ZEND_AST_METHOD_CALL) {
		...
		const zend_function *fbc = zend_hash_find_ptr(&ce->function_table, lcname);
		zend_string_release(lcname);
		return fbc && zend_fn_returns_nonnullable_scalar(fbc, scalar_type);
	}
...
static uint8_t zend_get_static_scalar_ast_type(zend_ast *ast) /* {{{ */
{
	...
	/* Unary minus/plus over a guaranteed-int operand is itself a guaranteed int: this is how
	 * negative int literals are written at all (`-5` parses as ZEND_AST_UNARY_MINUS over the
	 * int literal `5`), so it is literal notation, not arithmetic folding — `5 + 3` (BINARY_OP)
	 * and `~5` (a bitwise UNARY_OP) stay excluded. The operand must itself be a guaranteed int:
	 * `-9223372036854775808` is unary minus over a *float* literal (the magnitude exceeds
	 * ZEND_LONG_MAX, so the lexer already produced a float) and is correctly not classified, and
	 * unary +/- over a string/float is not a guaranteed int. Nested forms (`- -5`) recurse. */
	if (ast->kind == ZEND_AST_UNARY_MINUS || ast->kind == ZEND_AST_UNARY_PLUS) {
		return zend_get_static_scalar_ast_type(ast->child[0]) == IS_LONG ? IS_LONG : 0;
	}
