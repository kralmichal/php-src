...
/* Scalar-method backing-class submodules (str.c, int.c, float.c): their MINITs register the
 * internal Str/Int/Float classes. Declared here for BASIC_MINIT_SUBMODULE() below, mirroring
 * how the other ext/standard submodules are prototyped via their php_*.h headers.
 * (The *_arginfo.h headers are included by str.c/int.c/float.c themselves, not here.) */
