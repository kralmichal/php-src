...
/* {{{ Int::clamp(int $num, int $min, int $max): int
 * Constrains $num to [$min, $max]. Unlike abs()/pow(), the result is always an int (it is
 * one of $num/$min/$max, all ints), so clamp() is a CHAINABLE guaranteed-int method. */
PHP_METHOD(Int, clamp)
{
	zend_long num, min, max;
	...
	if (UNEXPECTED(min > max)) {
		zend_value_error("Int::clamp(): $max must be greater than or equal to $min");
		RETURN_THROWS();
	}
